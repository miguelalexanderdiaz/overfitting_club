...
The loss function translates the $-\text{ELBO}$ directly. Both terms are summed
over their respective dimensions (pixels for reconstruction, latent dims for KL)
and averaged over the batch. A weight $\beta$ controls the trade-off between
reconstruction fidelity and latent space regularity — this is the
**β-VAE** [@kingma2013auto] formulation:

```{python}
BETA = 0.005  # KL weight — balances reconstruction vs. regularity

def vae_loss(x, x_hat, mu, log_var, beta=BETA):
    """
    -ELBO = reconstruction (MSE) + β · KL divergence.

    Reconstruction:  Σ_pixels (x - x̂)² / batch_size
    KL(q ‖ p):      -½ Σ_dims (1 + log σ² - μ² - σ²) / batch_size
    """
...
Our loss sums over 784 pixel terms (reconstruction) and 32 latent-dimension
terms (KL). With $\beta = 1$ (the pure ELBO), each KL dimension gets the same
weight as each pixel. That sounds fair — but in practice, each per-dimension KL
value is **much larger** than each per-pixel MSE. The result: KL dominates,
reconstructions are blurry, and the encoder barely encodes.

**The "dimensionally balanced" β.** If we want the 49 KL terms to carry the same
*total* weight as if they were 49 out of 784 + 49 = 833 equal terms, we'd set:

$$
\beta_{\text{balanced}} = \frac{D_{\text{latent}}}{D_{\text{pixels}}} = \frac{49}{784} \approx 0.063
$$

**Our choice: $\beta = 0.005$** — intentionally ~12× smaller than the balanced
value. This favors reconstruction quality while keeping just enough KL pressure
to smooth the latent space. Here's the relative influence each term has in the
total loss at convergence:

| $\beta$ | Recon influence | KL influence | Reconstruction quality |
|:--------|:--------------:|:------------:|:----------------------|
| 1.0 (pure ELBO) | ~21% | **~79%** | Very blurry — KL crushes details |
| 0.041 (balanced) | ~86% | ~14% | Decent — slight softness |
| **0.005 (ours)** | **~98%** | **~2%** | Sharp — close to Conv AE |

Even at 2% influence, the KL term is enough to regularize the latent space into a
smooth, continuous manifold — which is all we need for generation. The key
insight: **you don't need much KL pressure to get a well-structured latent space**.
:::
...
# Left: reconstruction-only comparison (per-pixel MSE, apples to apples)
# FC/Conv AE already use nn.MSELoss (mean), VAE uses sum/batch → divide by 784
vae_recon_perpixel = [r / 784 for r in vae_recon_history]
for name, hist, color, dash in [
    ("FC AE", fc_history, "#94a3b8", "dot"),
    ("Conv AE", conv_history, "#10b981", "dot"),
    ("VAE (recon)", vae_recon_perpixel, "#6366f1", "solid"),
]:
    fig.add_trace(go.Scatter(
...
console.print(f"[bold blue]VAE final loss:[/bold blue] {vae_total_history[-1]:.4f} "
              f"(recon: {vae_recon_history[-1]:.4f}, "
              f"per-pixel: {vae_recon_history[-1]/784:.6f}, "
...
for name, hist, color, dash in [
    ("FC AE", fc_history, "#94a3b8", "dot"),
    ("Conv AE", conv_history, "#10b981", "dot"),
    ("Conv VAE", vae_recon_perpixel, "#8b5cf6", "dash"),
...
console.print(f"[bold yellow]ResNet VAE recon loss:[/bold yellow] "
              ...
              f"(Conv VAE: {vae_recon_perpixel[-1]:.6f})")
```
...
introduced by Kingma and Welling [@kingma2013auto], residual networks by He et
al. [@he2016deep], the autoencoder codec architecture from Taming Transformers
by Esser et al. [@esser2021taming], the latent diffusion approach by Rombach et
al. [@rombach2022high], and the Stable Diffusion 3 architecture by Esser et al.
[@esser2024scaling]. For a comprehensive survey of autoencoder methods, see Bank
et al. [@bank2023autoencoders].
